...
export default async function handler(req, res) {
  ...
  if (!toEmail) return res.status(500).json({ error: "QUOTE_TO_EMAIL is not configured" });
  if (!fromEmail) return res.status(500).json({ error: "QUOTE_FROM_EMAIL is not configured" });
